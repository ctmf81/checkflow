...
- **Execução offline**: marque "Disponível offline" para o checklist poder ser respondido **sem internet** pelo app instalado (aparece na lista da Operação mesmo sem sinal). Use em checklists simples para campo sem cobertura. Padrão "Só com internet". Checklists com plano de ação obrigatório ou de workflow precisam de conexão.
...
- **App instalável (PWA) + execução offline**: o CheckFlow pode ser **instalado na tela inicial** do celular (botão "Instalar" no topo da Operação; sem loja de apps). Instalado, a **Operação funciona sem internet** para os checklists marcados como "Disponível offline" na configuração do checklist. Como usar: o operador abre a Operação **uma vez com internet** (no escritório/depósito) para o app baixar esses checklists; em campo **sem sinal** eles aparecem na lista, podem ser preenchidos (inclusive com foto) e **finalizados** — a execução fica guardada no aparelho e é **enviada automaticamente quando a internet volta** (um indicador mostra "aguardando envio"/"enviando"). As respostas em andamento são salvas no aparelho, então não se perdem se a conexão cair. **Login precisa de internet** (faça login antes de ir a campo; a sessão fica salva). **Offline vale só para a Operação** — Gestão/Sistema exigem conexão. Execuções com **plano de ação, workflow ou agendamento** exigem internet para finalizar (use "Continuar depois").
