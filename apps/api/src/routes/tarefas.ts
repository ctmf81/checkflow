...
export async function tarefasRoutes(app: FastifyInstance) {
  ...
  app.post('/tarefas/notificar', async (req, reply) => {
    ...
    const destinatarios = new Map<string, { nome: string; telefone: string | null; telegramChatId: string | null }>()
    if (subgrupoIds.length > 0) {
      const { data } = await sb.from('usuario_subgrupo')
        .select('usuario_id, usuarios(nome, telefone, telegram_chat_id)')
        .in('subgrupo_id', subgrupoIds)
      for (const m of (data ?? [])) {
        const u = (m as any).usuarios ?? {}
        destinatarios.set((m as any).usuario_id, { nome: u.nome ?? '—', telefone: u.telefone ?? null, telegramChatId: u.telegram_chat_id ?? null })
      }
    ...
    await Promise.all(ids.map(async (uid) => {
      ...
      const { nome, telefone, telegramChatId } = destinatarios.get(uid)!
      ...
      const { ok, erro } = await enviarComFallback({ numero, telegramChatId, mensagem })
      if (ok) enviados++
