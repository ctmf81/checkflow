...
| Load Test (100 VU) | Node.js (simple) | ✅ 9.575 req, p95=237ms (2026-06-24) — throughput 319 req/s, 0% errors |
| Query N+1 Detection | Node.js + Supabase | ✅ 5/5 PASSED (2026-06-24) — FK embeds optimal, zero N+1 |
| Backup/Restore | Node.js + Supabase | ✅ 7/7 PASSED (2026-06-24) — RLS, triggers, ForeignKeys, cascade delete |
| Blue-Green Deploy | File inspection | ✅ 7/8 PASSED (2026-06-24) — feature flags, git rollback, zero-downtime ready |
...
### 🧪 Plano de teste GUIADO — `docs/qa/PLANO_TESTE_GUIADO.md` (EM ANDAMENTO, 2026-07-02)
Formato por tela: **funcionalidade · usuários necessários · respostas esperadas · riscos · exceções** + tabela de casos com status. Construído **incrementalmente** (uma tela por vez). Testado **em PRODUÇÃO** com usuários reais (criados via service role — credenciais na memória `teste-manual-guiado-progresso`). **Validadas: Telas 1–8 + Tela 9 parcial (9.1.1–9.1.5, 9.4.x turnos ✅).** Próxima: terminar Tela 9 (9.1.6, 9.2.x, 9.3.x, 9.5.x) → Tela 10 (Grupos/Subgrupos).
**Lição:** testar **como o usuário real** (ex.: logar como Admin da empresa / como operador puro, não impersonando) pegou **bugs reais** que impersonação/unit-test escondiam — incl. integração/infra (migration nunca aplicada, env, RLS self-select, stale state). Ver `/security` (achados) e a memória do progresso.
**Tela 9.4 (2026-07-02) — Turnos — 11 casos OK + 1 fix:**
Bug 22: turno `12x36` da empresa QA Smoke tinha `ativo = false` → `usuario_esta_no_turno()` retornava `true` por NOT FOUND (sem turno = sem restrição). Fix: `update turnos set ativo = true`. Aprendizado: debug query de ciclo escala deve incluir `hora_inicio` no ponto base (sem isso o pos calculado manualmente diverge do da função). Timezone do Supabase = UTC; função SQL também UTC.
**Tela 5 (2026-06-30) — 3 achados:** (1) operador não-admin travava em "Nenhuma unidade selecionada" → RLS self-select faltante em `usuario_empresa`/`usuario_grupo` (migration `20260630000000`, ver `/db`); (2) operador puro sem **logout** no header da Operação → menu de usuário com "Sair" (commit `8427336`); (3) decisão de produto: **execução sempre começa do zero** — removido o rascunho local (autosave/restauração), commit `e016ebb` (ver `/biz`).
**Tela 6 (2026-06-30) — casos 1–10 OK + 2 fixes** (commit `ae73ff6`): (1) **plano de ação obrigatório** ao reprovar item com `gera_plano_acao` (antes deixava finalizar sem); (2) **Finalizar leva à seção do campo pendente** (acordeão escondia o campo faltante). Checklist de teste rico **"QA Execução (Tela 6)"** criado em Linha 1 (tipos variados, validação, dependência, gera plano, foto, motivos). Catálogo/GPS/vídeo/assinatura/QR exigem aparelho → ficam p/ a Tela 7.
**Tela 7 (2026-06-30, no celular) — casos 1–9 OK + 2 fixes** (commit `c4857b1`): (1) **banner "sem conexão"** na lista (não existia); (2) **"Continuar depois" desabilitado offline** com dica (antes falhava em silêncio — exige servidor). Idempotência (caso 9) confirmada no código. Pendência mapeada (deixar como está): **fila de sync presa ao aparelho** (IndexedDB) — sobrevive ao logout, sincroniza quando um operador abre a Operação nesse device online. Ver `/biz`.
...
### Pen Test — Causa raiz na abertura do plano (`pentest/causa-raiz-rls.mjs`)
6 checagens das migrations 20260622190000/200000/210000: N1/N2 do subgrupo **criam** causa raiz (resolvedor, sem permissão de gestão); operador comum (funcao 'operacao') **não** cria; gestor com permissão `causa_raiz` cria; membro da unidade registra `causa_raiz_ocorrencias`; sem escrita cross-unidade; leitura escopada à unidade. ⚠️ **Pegou bug real**: `usuario_subgrupo` não tinha self-read (igual ao caso `usuario_unidade_propria`) → quebrava a policy resolvedor + o gating `ehResolvedor` no front. Fix: `20260622210000_fix_usuario_subgrupo_select_propria.sql`. Rodar: creds dos `.env` + `node pentest/causa-raiz-rls.mjs`.
...
### ✅ Unit — Painéis de Dashboard — `tests/unit/lib/painelDados.unit.test.ts` (30 testes, 2026-07-09; **+painel de checklist 2026-07-11**)
Lógica pura extraída da rota `/api/painel/[token]` para `lib/painelDados.ts` (fonte única, importada pela rota). Cobre:
- **Painel de atividade**: `num` (número/string/objeto-padrão/nulos/não-numérico), `tendencia` (não-conformidade 1ª vs 2ª metade: alta/queda/estável + limiar 5pp + metade vazia), `opcoesSimNao` (conformidade pelo `esperado`), `montarLinha` (série filtra não-numéricos; ref do config p/ número e da resposta mais recente p/ padrão), `montarBarras` (contagem por opção, não-conformes por `e_valido`, ignora nulo/objeto/array), `agruparPorDia`/`serieConformidade`/`composicaoDiaria`/`montarPadrao`, `resumoExecucao` (execuções concluídas × não executadas + motivos agrupados, "Sem motivo" no vazio).
- **Painel de checklist (2026-07-11)**: `placarChecklist` (aprovados/reprovados/não exec. + % aprovação; resultado null = aprovado; sem execução = pct null), `conformidadePorDiaExec` (aprovado×reprovado por dia, ignora não-concluídas), `tempoMedioExecucao` (média das concluídas com `iniciado_em`; ignora sem-início e não-concluídas; null sem amostra), `topNaoConformes` (agrupa por atividade via `conforme`, filtra as sem NC, ordena desc), `resumoPlanos` (contagem por status).
