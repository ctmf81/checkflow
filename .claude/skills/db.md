...
⚠️ **`indicadores` e `relatorios` removidos do construtor** (`permissoes.ts`, 2026-06-30, commit `63d9659`): nunca existiram em `permissoes` (marcar não salvava — bug reportado) e não têm enforcement próprio. **Menu "Relatórios"/"Dashboards"** apontam p/ páginas inexistentes (404) — **planejadas**, não remover.
...
### Serviços / Entitlements (migration `20260709050000_servicos.sql`, ⏳ aplicar)
- `servicos` (chave unique, nome, descricao, tipo `modulo|caracteristica`, `recursos text[]`, flag, ordem, ativo) + `plano_servicos` (plano_id × servico_id, PK composta). RLS: SELECT p/ authenticated (catálogo, necessário p/ gating/comparação); escrita `is_admin_sistema`. Seed do catálogo padrão. Empresa herda serviços do `empresa_assinaturas.plano_id`. Gating de UI em `SessionContext.recursosHabilitados`. Ver `/biz`.

### Dashboards — painéis públicos de TV (migration `20260709030000_dashboards.sql`, ⏳ aplicar)
- `dashboards` (unidade_id, nome, `token` único default `encode(gen_random_bytes(16),'hex')`, transicao_segundos, refresh_segundos) + `dashboard_paineis` (dashboard_id, ordem, titulo, `atividade_id` → checklist_atividades, janela_horas). Permissão `dashboards` (ver/criar/deletar), seed p/ perfis `is_system`. RLS: leitura por membro da unidade / admin-empresa; escrita por `usuario_tem_permissao('dashboards','criar')` + unidade (ou admin). **Leitura pública NÃO usa RLS** — a rota `/api/painel/[token]` usa service-role e é escopada pelo token. Ver `/biz`, `/security`.
- **Performance (2026-07-09)**: leitura leve (SELECT indexado, MVCC não trava o transacional). Salvaguardas: migration `20260709040000` = índice composto `checklist_execucao_respostas(atividade_id, criado_em)` (range scan da janela) + **cache em memória de 15s por token** na rota (colapsa N TVs do mesmo dashboard num hit + anti-abuso do link público). Escala futura = **read replica do Supabase** apontando só a rota pública (não banco à parte).
