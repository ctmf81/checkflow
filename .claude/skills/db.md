...
- ⚠️ **Trigger `planos_acao_set_numero_seq` usava `g.empresa_id` (coluna inexistente)** — `grupos` NÃO tem `empresa_id` (cadeia é grupos→unidades→empresas). A migration `20260606000013` redefiniu a função com `g.empresa_id`; em prod foi corrigido à mão, mas em DEV (montado só das migrations) o trigger quebrava e **todo insert em `planos_acao` falhava** com "column g.empresa_id does not exist". Corrigido por `20260731120000_fix_planos_acao_numero_seq` (cadeia certa + volta a preencher `identificador` PA-AAAAMM-XXXX). Aplicado em dev e prod. (descoberto 2026-07-31)
...
| Table | Description |
|-------|-------------|
| `parceiros` | `nome, email (unique lower), telefone, documento (CPF só dígitos, unique parcial — chave de busca na UI), status status_geral default 'ativo', email_boasvindas_enviado_em, `**`asaas_wallet_id`**` (subconta p/ split real, `20260722130000`), `**KYC** (`20260723120000`): `data_nascimento` (PF), `tipo_empresa` (PJ, check MEI/LIMITED/INDIVIDUAL/ASSOCIATION), `renda_mensal`, `cep`, `endereco`, `endereco_numero`, `complemento`, `bairro`, ` criado_em, criado_por, atualizado_em`. Um parceiro pode estar vinculado a várias `empresas`. Split: ao assinar/reativar plano pago, se wallet + `empresa_financeiro.parceiro_percentual>0` → `split` na assinatura Asaas. Subconta criada por `POST /parceiros/:id/conta-asaas` (usa o KYC) |
...
| `parceiro_pre_cadastros` (`20260723140000`) | Captação pública de parceiros. `nome, documento (CPF/CNPJ dígitos), email, telefone, mensagem, status pendente/aprovado/rejeitado, parceiro_id (FK, na aprovação), criado_em, moderado_por, moderado_em` + **KYC preenchido pelo próprio interessado** (`20260723160000`): `data_nascimento, tipo_empresa (check), cep, endereco, endereco_numero, complemento, bairro` — copiados para `parceiros` na aprovação (sem `renda_mensal`, que é mockada). RLS: INSERT anon só `status='pendente'`; SELECT/UPDATE `is_admin_sistema()`. Form público `/seja-parceiro`; validação em `/sistema/parceiros` (aprovar → cria `parceiros`) |
