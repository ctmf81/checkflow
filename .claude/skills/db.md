...
| Table | Description |
|-------|-------------|
| `parceiros` | `nome, email (unique lower), telefone, documento (CPF só dígitos, unique parcial — chave de busca na UI), status status_geral default 'ativo', email_boasvindas_enviado_em, `**`asaas_wallet_id`**` (subconta p/ split real, `20260722130000`), criado_em, criado_por, atualizado_em`. Um parceiro pode estar vinculado a várias `empresas`. Split: ao assinar/reativar plano pago, se wallet + `empresa.parceiro_percentual>0` → `split` na assinatura Asaas (repasse automático da mensalidade) |
