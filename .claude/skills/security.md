...
| 2026-08-18 | **Perfil "Gestão do Grupo" bloqueado silenciosamente** — a UI oferecia as permissões `grupos.adicionar_usuario`/`gerenciar_usuario`, o admin da empresa concedia ao perfil, mas a RLS de `usuario_grupo`/`usuario_subgrupo` só checava `admin_sistema`/`admin_empresa`. INSERT bloqueado sem erro visível — modal "Adicionar usuário" fechava como se tivesse salvo mas nada acontecia. **Não era vazamento**, era broken-access na direção oposta (feature travada). | `20260818095330` — policies `usuario_grupo_permissao`/`usuario_subgrupo_permissao` que honram as permissões E escopam por `usuario_empresa` (bloqueia cross-empresa). Front (`AdicionarUsuarioModal`/`UsuariosGrupoModal`): `.select()` no upsert + tratamento de `error`/array vazio → nunca mais fecha em silêncio. **Lição: ao criar permissão na UI de perfis, verificar que a RLS realmente honra `usuario_tem_permissao(recurso, acao)` — o checkbox não faz nada sozinho.** |
| 2026-08-18 | **Cron `/cron/gestao/lembretes` avisava empresa inativa** — filtrava `pre_cadastros.status='pendente'` mas não olhava `empresas.status`. Admin desativava a empresa e continuava recebendo lembretes de pré-cadastros pendentes | `avisos-gestao.ts` — join com `empresas!inner` + `.eq('empresas.status', 'ativo')`. **Lição: qualquer cron que gera notificação a admins precisa filtrar por status da empresa (avisos-trial e avisos-uso já filtravam via `empresa_assinaturas` — só o de gestão estava exposto).** |
