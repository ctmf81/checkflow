...
Relatório: `docs/seguranca/RELATORIO_SEG_PERF_2026-07-19.md`. Suítes dinâmicas contra prod: `run.mjs` 46/49, RLS suites (documentos/causa-raiz/billing-templates/admin-empresa) OK, `scale-rls-100` 6/6, `backup-restore` 7/7, `query-n1` 5/5 (sem N+1), `http_probe` 24/26. **Cobertura de RLS = 100%** das tabelas (verificado ao vivo: anon lê 0 de planos_acao/workflows apesar de dados reais). Os "fails" do run.mjs são a **galeria de templates lida por anon** (não é vazamento de tenant nem bypass de JWT). Achados abertos (⏳ corrigir): **MEDIUM SSRF** em `usuarios.ts:51` `fetch(empresa.importacao_api_url)` sem validação de host nem timeout (mitigado pelo cron-secret; validar IP privado/metadata + `AbortSignal.timeout`); **LOW** galeria de templates legível por anon (policy `or is_template` sem role guard); **LOW** injeção de HTML nos templates de e-mail (interpolação crua). ⏳ `load-test-simple`/k6 NÃO rodados (flood em prod compartilhada — rodar em staging).
