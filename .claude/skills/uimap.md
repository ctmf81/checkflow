...
| `lib/usePolling.ts` | Hook de **polling** das listagens: `usePolling(carregar, 45000, enabled)` — refetch periódico, **pausa em aba oculta** + refetch ao voltar. Escolha vs Supabase Realtime = custo previsível no free (ver `/biz`). Aplicado nas listagens da Gestão (Home/tickets/planos-acao/tarefas/checklists/usuarios/perfis/turnos/agendamentos/grupos/subgrupos/documentos/catalogos/nao-execucao/causa-raiz/dashboards/padroes/variaveis). **Não** em forms/detalhe, operação (offline) nem sistema |
...
| `lib/entitlements/assinaturaFase.ts` | Regras da **fase da assinatura**: `podeCriarConteudo` (só cria na fase `ativa` — pós-trial=somente leitura; usada nas 5 telas de criação) + `estadoAssinaturaGate` (decisão do `AssinaturaGate`: nada/bloqueio_total/banner) + `MSG_CRIACAO_BLOQUEADA`. + testes |
...
| `app/api/empresas/[id]/excluir/route.ts` | **Exclui empresa + arquivos** (service role, só admin de sistema, empresa inativa): enumera IDs via unidade→empresa e remove objetos dos buckets `execucoes`/`empresas` antes de chamar a RPC `excluir_empresa_cascata` (via JWT do admin). Best-effort no storage. `ExcluirEmpresaModal` chama esta rota. Ver `/biz` |
...
| `lib/tiposAtividade.ts` | Tipos de atividade do montador + **gate por serviço do plano** (`TIPOS_ATIVIDADE` c/ `recurso`/`flag`; `tiposAtividadeDisponiveis`). Importada por `AtividadeModal`; catálogo gateado por `catalogos`. + testes |
| `lib/tarefaIndicadores.ts` · `lib/empresaExclusao.ts` | Lógica pura extraída (indicadores de tarefas: feito×não/média/evidências/pontos; exclusão de empresa: path do logo/prefixos de storage) — importadas pela página de indicadores / rota de exclusão + testes |
...
| `lib/ia/interpretarFoto.ts` | Lógica pura da IA por foto (compor prompt por tipo + normalizar resposta: `comporPromptFoto`/`normalizarSimNao`/`extrairNumero`/`posProcessarFoto`), usada por `/api/ia/interpretar-foto` + testes |
| `app/api/ia/interpretar-foto/route.ts` | **IA por foto no campo** (texto/sim_não/número): recebe imagem+atividadeId → gate característica `ia` + cota tokens → failover multimodal (`ia_provedores`) → `{ valor }`. Componente de captura = `CampoIAFoto` em `operacao/[id]` (foto vira evidência `{valor,foto_ia}` no finalize). Ver `/biz` (IA por foto no campo) |
