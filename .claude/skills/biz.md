...
- **Serviço "padrão"** (`servicos.padrao`, migration `20260709070000`): funções base **sempre disponíveis** (independem do plano) — seed marca `checklists`, `estrutura` (grupos/áreas), `catalogos`. Os recursos desses serviços entram no conjunto liberado mesmo com plano restrito. Toggle em `/sistema/servicos` (só p/ módulo); na comparação aparecem como "incluído em todos".
...
- **Gráficos** (SVG na página): **número/padrão** → linha do tempo das últimas X horas + linha(s) de referência (número = min/max do `config`; padrão = `valor_min/valor_max` da resposta mais recente) + valor atual grande, vermelho se fora da faixa. **sim_não/única escolha** → barras por opção (verde=conforme/válida, vermelho=não) + **tendência da não-conformidade** (1ª metade vs 2ª metade da janela: alta=piorando/vermelho, queda=melhorando/verde) + contador de não-conformes.
