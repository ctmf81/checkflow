...
- **Criar a subconta pelo próprio CheckFlow (self-service)**: em `/sistema/parceiros`, botão **"Criar conta Asaas"** → `POST /parceiros/:id/conta-asaas` (**admin_sistema**) cria a subconta white-label (`asaasCriarSubconta` → `POST /accounts`) reusando nome/e-mail/documento/telefone do cadastro e grava o `walletId` sozinho. Idempotente (não recria se já há wallet). Com wallet, a UI mostra "Conta Asaas ativa (split ligado)".
  - ⚠️ **Limites de KYC**: a subconta pode precisar completar dados/verificação no Asaas antes de **receber** de fato. O cadastro do parceiro só tem nome/e-mail/documento/telefone; campos extras que o Asaas exigir (endereço, `incomeValue`, `birthDate`, `companyType`) fazem o `POST /accounts` retornar **erro descritivo**, repassado ao admin na tela. CNPJ vai com `companyType: 'LIMITED'` por padrão (editável na subconta).
