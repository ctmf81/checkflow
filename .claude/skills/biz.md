...
- **Seção 03 "prova" (revisada 2026-07-30)**: comunica **robustez validada em ambientes industriais de pequeno e grande porte** (sem citar tempo de validação — o "1 ano/beta" foi retirado por soar amador). Traz um **carrossel de funcionalidades** (`.fcar`/`.fcard`, scroll-snap horizontal, cabeçalho empilha em <=640px) mostrando o que o produto entrega: checklists com prova, tickets, tarefas instantâneas, turnos, plano de ação, causa raiz, **mensageria instantânea** (avisos WhatsApp/e-mail/push p/ não conformidade e necessidade de ação), agendamento, relatórios IA, leitura de imagem IA, offline/PWA, painéis de TV, documentos/Consulta IA. Cards de IA levam tag "IA". Ao adicionar funcionalidade nova, incluir um card aqui. ⚠️ Este HTML é **estático e mantido direto em `apps/web/public/`** — desde 2026-07-30 existe tanto em `main` quanto em `develop` (antes só na main → 404 no dev), então edite pelo fluxo feature→develop→main normal.
...
- **Criar a subconta pelo próprio CheckFlow (self-service)**: em `/sistema/parceiros`, botão **"Criar conta Asaas"** → `POST /parceiros/:id/conta-asaas` (**admin_sistema**) cria a subconta white-label (`asaasCriarSubconta` → `POST /accounts`) reusando nome/e-mail/documento/telefone do cadastro e grava o `walletId` sozinho. Idempotente (não recria se já há wallet). Com wallet, a UI mostra "Conta Asaas ativa (split ligado)".
  ...
  - **O que dá para editar de um parceiro** (`ParceiroEditarModal`, admin_sistema, botão "Editar" sempre visível): **nome, e-mail, telefone, CPF/CNPJ, status (ativo/inativo), Wallet ID** + todo o KYC. A **renda é omitida** do modal (sempre mockada).
  - **Documento IMUTÁVEL — "teve wallet um dia, CPF nunca mais muda"** (migration `20260724100000`): a coluna `parceiros.asaas_wallet_criada_em` é carimbada automaticamente (trigger) na 1ª vez que `asaas_wallet_id` fica preenchido — por qualquer caminho (rota `conta-asaas`, edição manual do Wallet ID) — e **nunca volta a null**. Tendo carimbo, o **banco rejeita** qualquer UPDATE que mude `documento` (exception no trigger `parceiros_documento_imutavel`), então a regra vale para a tela, a API e escrita direta. Limpar o Wallet ID **não** destrava. A UI lê o carimbo (e o espelha localmente ao criar a conta, para travar sem recarregar).
  - **Mudar status ou wallet re-sincroniza o split** de TODAS as empresas vinculadas (chama `/billing/sincronizar-split` para cada). Sem isso, desativar um parceiro deixaria o dinheiro continuar sendo repassado nas assinaturas vigentes.
  - **Excluir parceiro**: botão no modal com confirmação. **Bloqueado se houver empresa vinculada** (`empresa_financeiro.parceiro_id` é ON DELETE SET NULL — apagaria o repasse em silêncio); a mensagem lista as empresas. A subconta no Asaas **não** é apagada. Campos compartilhados em `ParceiroKycFields.tsx`, que também **completa Endereço e Bairro pelo CEP** (ViaCEP, ao digitar os 8 dígitos ou no blur; `logradouro`→endereco, `bairro`→bairro; CEP inexistente/API fora só avisa e deixa preencher à mão).
...
- **Fluxo**: interessado acessa o **formulário público `/seja-parceiro`** (link/QR gerado no botão "Formulário de captação" em `/sistema/parceiros`), preenche nome/CPF-ou-CNPJ/e-mail/telefone/mensagem **+ o KYC completo** (nascimento se PF ou tipo de empresa se PJ, CEP/endereço/nº/complemento/bairro — reusa `ParceiroKycFields` com `ocultarRenda`, incluindo o autopreenchimento por CEP) → cria registro **PENDENTE** em `parceiro_pre_cadastros` (migration `20260723160000` adicionou as colunas de KYC lá). **Na aprovação, o KYC é copiado para `parceiros`** — a subconta Asaas já nasce completa, sem o admin perseguir dado depois. O admin de sistema **valida** na própria tela de Parceiros (seção "Interessados aguardando validação"): **Aprovar** cria um `parceiros` (status ativo) e marca o pré-cadastro `aprovado` (com `parceiro_id`); **Rejeitar** marca `rejeitado`. Depois é só **associar o parceiro à empresa** (aba Parceiro de `/sistema/empresas/[id]`).
...
- **Cards de execução (2026-07-30)**: tempo relativo e badge de status ficam **lado a lado no desktop** (`sm:flex-row`) e **empilhados no mobile** — antes o badge sempre caía numa 3ª linha.
...
- **Empresa de demonstração** (2026-07-30): se a empresa é demo (`empresas.demo`, marcado pelo admin de sistema em `\sistema\empresas\[id]`→Config, junto da vertical), aparece o card **"Empresa de demonstração"** com o botão **"Gerar dados dos últimos 30 dias"**. Clica → `POST /api/empresa/demo/gerar` provisiona a vertical (1x) e **ACRESCENTA** massa fake (execuções, planos, tickets, tarefas) na janela de 30 dias — **append, não apaga**. Como a Home/Indicadores filtram por período, os dados sempre "aparecem frescos" e o que passa de 30 dias some das telas. Verticais: fábrica de alimentos, gestão de condomínios (registry `lib/demo/verticais`). Empresa demo é **isenta de billing** (nunca bloqueia). Ver `/db`, `/uimap`. **Ao criar funcionalidade nova que gere dado, considerar refletir no gerador.**
...
- **Rota** `/api/ia/interpretar-foto` (não-streaming): busca tipo/prompt da atividade server-side; **gate característica `ia` + `billing_pode_consumir_ia`**; failover; debita `uso_ia_eventos`. Retorna **`{ valor, tipo, provedor }`** — `tipo` (do config da atividade) e `provedor` (nome de quem gerou) são metadata pro cliente escalar/logar; o campo salvo é só `valor`.
- **Execução** (`operacao/[id]`, `CampoIAFoto`): captura → rota → preenche o campo (escalar no path ao vivo). Foto **guardada como evidência** (bucket `execucoes`, cota de armazenamento) no finalize → **o CLIENTE** empacota a resposta em **`{ valor, foto_ia }`** ao gravar em `checklist_execucao_respostas` (o `foto_ia` é a URL da evidência subida). `calcularValidacao` desempacota (valida pelo valor); viewer + PDF idem; viewer/execução mostram a foto (lightbox). `ia_editavel=false` trava o campo (valor read-only). Exige conexão (toggle no montador some se checklist é `permite_offline`).
