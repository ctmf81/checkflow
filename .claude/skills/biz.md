...
- **Data de liberação** (`tarefa_listas.liberacao_em`, nullable): quando a lista publicada passa a **aparecer na Operação**. Vazio = libera ao publicar (comportamento antigo). No futuro → lista fica **agendada** e é **ocultada do operador** até a data (barreira `liberada()` em `lib/tarefas.ts`, antes da janela de abertura). Campo data+hora no montador.
- **Status derivado** (`statusTarefa()` em `lib/tarefas.ts`) para a listagem da gestão: `rascunho` (não publicada) · `agendada` (publicada, liberação no futuro) · `em_execucao` (liberada + janela de abertura aberta) · `finalizada` (encerrada manual OU janela de abertura fechada por data limite/nº respostas). Filtro por chips (Todas/Rascunho/Agendada/Em execução/Finalizada) + badge na lista usa esse status derivado (não mais o cru rascunho/publicada/encerrada).
