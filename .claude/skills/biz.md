...
## Admin da empresa — mesmas funções do admin de sistema, escopadas (2026-06-20)
...
- **Pode (dentro da SUA empresa)**: gerenciar usuários/acessos (incl. **atribuir outro Admin da empresa**), estrutura (unidades, grupos, subgrupos, turnos), perfis não-sistema e permissões; e **"vê tudo"** — ignora o filtro por subgrupo nas telas operacionais (tickets, planos de ação, tarefas, operação, agendamentos), igual o admin de sistema faz, porém só na empresa.
...
- **Implementação**: migration `20260620120000_admin_empresa_rls.sql` — helpers `is_admin_empresa(empresa_id)`, `is_admin_empresa_unidade/_grupo/_subgrupo` + policies **aditivas** (RLS combina com OR; não reescreve as existentes). UI: helper `lib/admin.ts` `ehAdminDaEmpresa(supabase, empresaId)` substitui o check de `role==='admin_sistema'` nas telas (retorna true p/ admin de sistema OU admin da empresa ativa).
- **Fase 2 pendente**: visibilidade operacional **cross-unidade** (ver dados de unidades onde o admin não é membro) ainda depende do RLS de leitura por `usuario_unidade`. Hoje o admin da empresa vê tudo das unidades a que pertence; para ver TODAS as unidades da empresa sem ser membro, falta estender as policies de leitura das tabelas operacionais + ajustar o `SessionContext` p/ listar todas as unidades. Ver `/status`.
