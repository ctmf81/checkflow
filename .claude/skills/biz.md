...
- **Filtro de unidade nas listagens (2026-06-20)**: TODA **listagem** deve ter, no mínimo, um **filtro de unidade**. Componente `components/ui/FiltroUnidade.tsx` — **ligado ao seletor global** (troca a `unidadeAtiva` do app inteiro) e **só aparece para quem tem +1 unidade**. ⚠️ **Dashboards NÃO** (ex.: Home/Visão Geral) — usam só o seletor global do header, p/ não duplicar. Aplicado em: Planos de Ação, Checklists, Tarefas, Tickets, Agendamentos, Catálogos, Documentos. **Pendente** nas demais listagens (Indicadores, Padrão, Acessos, Config: não-execução/causa-raiz/relatórios/dashboards/formatação/notificações).
