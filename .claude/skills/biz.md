...
- **Status**: aberto → em_tratamento → (aguardando_informacao = responsável pediu algo ao abridor, SLA pausa | aguardando_validacao = responsável propôs conclusão, espera o abridor validar) → corrigido / corrigido_parcialmente / nao_corrigido / cancelado / improcedente.
...
- ⏳ **Pendente (a fazer com cuidado): respeitar perfil `ticket.ver`/`criar`/`tratar`** — enforcement precisa de migration concedendo essas permissões aos perfis que já usam tickets, senão trava usuários atuais (FAB "Abrir Ticket" da Operação). Hoje a visibilidade é por subgrupo (acima) e as ações por papel/membership.
