...
- 🔴→✅ **Fix RLS (migration `20260618140000_tarefas_admin_exec.sql`)**: admin de sistema não conseguia abrir/responder lista de tarefas (insert de `tarefa_execucoes` exigia `usuario_unidade`). Adicionado bypass `is_admin_sistema()` no `tarefa_exec_insert`. **Pendências**: (1) envio WhatsApp ao publicar não wired (só flag); (2) mídia de tarefa não conta na cota de armazenamento; (3) testar e2e após aplicar migration.
