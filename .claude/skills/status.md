...
- ✅ **Tickets revisado/corrigido 2026-06-18**: listagem filtra por subgrupo do usuário (+ os que abriu; admin vê todos); "Assumir" só para membros do subgrupo de destino; terminologia "setor"→subgrupoLabel no detalhe. SLA cadastrado em `/gestao/tickets/sla` (por prioridade + override por categoria). ⏳ **pendente (cuidado): respeitar perfil `ticket.ver/criar/tratar`** — precisa de migration concedendo aos perfis atuais p/ não travar (FAB Abrir Ticket na Operação).
...
- ⏳ **Ordem da revisão a seguir**: Planos de ação → Catálogos → Documentos → Indicadores → **(por último) Workflows**.
