...
- ✅ **Workflows revisado/corrigido 2026-06-18**: (1) "porta dupla" — checklist liberado como item de workflow some da lista avulsa (executar solto não vinculava nem avançava o workflow); (2) "Workflows em andamento" filtra por subgrupo do operador (admin vê todos) — cada setor vê só a sua etapa; (3) subgrupo do item de workflow virou **obrigatório ao publicar**. Sequência entre setores já era garantida pelo motor (libera estágio a estágio). Em `operacao/page.tsx` + `workflows/[id]/page.tsx`.
- ⏳ Próxima tela da revisão: a definir (candidatas: Agendamentos, Tickets, Planos de ação, Catálogos, Documentos, Indicadores).
