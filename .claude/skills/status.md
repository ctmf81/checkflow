...
- ✅ **Tickets revisado/corrigido 2026-06-18**: listagem filtra por subgrupo do usuário (+ os que abriu; admin vê todos); "Assumir" só para membros do subgrupo de destino; terminologia "setor"→subgrupoLabel no detalhe. SLA cadastrado em `/gestao/tickets/sla` (por prioridade + override por categoria). **Decisão: NÃO usar perfil em tickets** (acesso por subgrupo + papel basta; só `ticket.cancelar` segue gateando improcedente/cancelar).
