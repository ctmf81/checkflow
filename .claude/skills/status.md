...
1. Home `/gestao` ✅ (dashboard só de checklist; escopo unidade ativa; funil por execução + indicador moderação N1/N2; SLA removido)  2. Grupos ✅  3. Tickets→Chamados 🟡 · Categorias ✅ (padrão "Não informada"; obrigatória no ticket; gerir por permissão `ticket/categorias_gerir`+unidade) · Config.SLA ❌
...
- ✅ **Gestão → Checklists (listagem + montador) revisada 2026-06-17**. Regras gravadas em `/biz` + MANUAL. Mudanças de código: tempo de guarda **64→60** + texto correto (apaga só mídias, impacta cota); **Inativar** agora pede confirmação + guard de workflow publicado (lista nomes, bloqueia); **Duplicar** copia motivos e **recria catálogos no destino** (cross-unidade, com aviso+confirmação) remapeando `config.catalogo_id`.
- ✅ **Limites de mídia (fixos) 2026-06-17** em `operacao/[id]/page.tsx`: `MAX_FOTOS=5`, vídeo `MAX_VIDEO_SEG=10`+bitrate 1,5Mbps (auto-stop), compressão `comprimirImagem` (1600px/JPEG 0.8). Plano de ação: até 5 fotos (botão some+aviso, contador). Atividade tipo **foto = 1 foto** (decisão fechada). Ver `/biz` "Limites de mídia".
- ✅ **Tipos de atividade revisados** 2026-06-17: data/hora pré-preenche horário atual (`CampoDataHora`); aviso QR/barcode mobile-only no montador; premissa "pensar no tipo de resposta, não na pergunta" + dependências (gravado no MANUAL/biz). Atividade foto = 1 foto.
- ✅ **Grupos/Subgrupos revisado** 2026-06-17: funções por subgrupo (—/Operação/N1/N2 + fluxo de moderação e notificação por nível), premissa "usuário precisa estar na empresa antes de entrar no grupo". **3 bugs corrigidos no `UsuariosGrupoModal`**: (1) reenviar senha agora usa `/api/usuarios/resetar-senha` (código WhatsApp, não mais `resetPasswordForEmail`); (2) remover do grupo limpa `usuario_subgrupo`; (3) editar ganhou seletor de **perfil** (com guard do último admin).
- ✅ **Acessos → Usuários revisado/corrigido 2026-06-17**: criação avulsa não vinculava empresa/perfil/unidades e edição não salvava perfil/unidades → corrigido (rota `/api/usuarios/criar` recebe empresaId/perfilId/unidades+rollback; modal salva perfil+unidades e carrega unidades atuais). Avatar removido. ⚠️ Nota: RLS de `usuario_empresa`/`usuario_unidade` é admin_sistema-only — edição client-side segue padrão do `alterarPerfil`; avaliar policy p/ admin da empresa.
- ⛔ **Workflows — DESABILITADO na UI 2026-06-18** (flag `WORKFLOWS_HABILITADO=false` em `apps/web/lib/features.ts`): fora do menu, telas mostram "indisponível", some da Operação/Agendamentos/Perfis. Código/dados intactos; `true` reativa. Tema em estudo. **Revisão da tela suspensa enquanto off.**
- 🟡 (histórico) Workflows — revisão pausada (PR #11 com correções já no ar). Já saíram correções (PR #11): (1) "porta dupla" — checklist liberado como item de workflow some da lista avulsa; (2) "Workflows em andamento" filtra por subgrupo do operador (admin vê todos); (3) subgrupo do item obrigatório ao publicar. Sequência entre setores garantida pelo motor. O usuário vai **testar antes** de fechar a revisão dessa tela. Em `operacao/page.tsx` + `workflows/[id]/page.tsx`.
- ✅ **Agendamentos revisado/corrigido 2026-06-18**: pendência agendada de checklist na Operação filtra por **subgrupo do checklist** (admin vê todas) — não mais qualquer operador; **edição** de agendamento habilitada (modal create/edit). Confirmado: workflow agendado chama `workflow_iniciar`; sem catch-up (próximo slot futuro); criar/editar via permissão `agendamentos` (RLS). ✅ listagem da Gestão filtra por subgrupo do gestor (admin vê todos) — checklist pelo subgrupo, workflow pelos subgrupos dos itens.
- 🟡 **Tickets — revisão PENDENTE (a confirmar após testes do usuário).** Correções já no ar (PRs #16/#17/#18). Listagem filtra por subgrupo do usuário (+ os que abriu; admin vê todos); "Assumir" só para membros do subgrupo de destino; terminologia "setor"→subgrupoLabel no detalhe. SLA cadastrado em `/gestao/tickets/sla` (por prioridade + override por categoria). **Decisão: NÃO usar perfil em tickets** (acesso por subgrupo + papel basta; só `ticket.cancelar` segue gateando improcedente/cancelar).
- ✅ **Tickets — fluxo simplificado 2026-06-18**: removida a etapa "Propor conclusão"/aguardando_validacao; o responsável **conclui direto** (corrigido/parcial/não corrigido) e o abridor é notificado (evento `conclusao`) e pode **reabrir** se discordar. Bloco de aguardando_validacao mantido só p/ tickets legados.
- ⏳ **Ordem da revisão a seguir**: Planos de ação → Catálogos → Documentos → Indicadores → **(pendentes de teste, por último) Tickets, Workflows**.
- ✅ **Listas de Tarefas — IMPLEMENTADO 2026-06-18** (ver `/biz`). Migration `20260618120000_tarefas.sql` ✅ aplicada 2026-06-18. 6 tabelas + permissão `tarefas` + RLS. UI: menu "Tarefas", Gestão `/gestao/tarefas` (+`[id]` montador, modal indicadores), Operação 4ª aba (`operacao/AbaTarefas.tsx`). Type-check ✅. **Testes unitários: `tests/unit/lib/tarefas.unit.test.ts` 23/23 ✅** (lógica pura em `lib/tarefas.ts`, importada pelo componente). **Admin de sistema vê todas as tarefas** (bypass do filtro grupo/subgrupo em `visivelPara`, pois não tem vínculos) — 2026-06-18. Mesma lógica aplicada à aba **Documentos** (admin "pertence" a todos os grupos/subgrupos da unidade). Aba **Checklists** agora **filtra por subgrupo** (2026-06-18): operador vê só os checklists dos subgrupos a que pertence (`usuario_subgrupo`); admin vê todos. **Checklist é obrigatoriamente associado a um subgrupo** (validação no publicar; sem subgrupo não aparece p/ operador). Histórico é só do próprio usuário.
- 🔴→✅ **Fix RLS (migration `20260618140000_tarefas_admin_exec.sql` ✅ aplicada)**: admin de sistema não conseguia abrir/responder lista de tarefas (insert de `tarefa_execucoes` exigia `usuario_unidade`). Adicionado bypass `is_admin_sistema()` no `tarefa_exec_insert`. **Pendências**: (1) ~~envio WhatsApp~~ ✅ **wired 2026-06-18** (rota `POST /tarefas/notificar` em apps/api → WhatsApp aos membros dos subgrupos/grupos, respeitando turno; chamado no publicar quando flag on); (2) ~~mídia de tarefa na cota~~ ✅ **2026-06-18** (origem 'tarefa' em `uso_armazenamento` via migration `20260618160000` ✅ aplicada + `registrarUsoArmazenamento` + bloqueio `billing_armazenamento_disponivel`); (3) testar e2e.
...
6. ✅ **Troca de plano agendada (2026-06-15)**: toda troca entre pagos vale no fim do período (`proximo_plano_id`+`troca_efetiva_em`); pago→pago PUT no Asaas (novo valor próxima cobrança), pago→gratuito cancela; 1ª contratação imediata. UI mostra banner "troca agendada" + confirma. Testado (pentest/billing.mjs 21/21)
7. ⏳ **IA por plano (adiado)**: flag `ia_habilitada` no plano — se off, esconder campo de tokens e bloquear ações de IA (Consulta Inteligente `/api/documentos/consultar`, futuros relatórios inteligentes) via `billing_pode_consumir_ia`. Gate central já existe; pontos de IA chamam `/api/documentos/consultar` (operacao/page.tsx:649). NÃO implementado.
8. ⏳ **Definir UX quando os tokens acabam**: hoje o enforcement só BLOQUEIA a ação de IA. Decidir a jornada: avisar admin? oferecer compra de pacote de tokens na hora? aviso antecipado ao se aproximar do limite? — a resolver.
9. ⏳ Fase 4: split parceiro via subconta Asaas (`asaas_wallet_id`, criação automática, split nas cobranças). Decidir: walletId manual (recomendado) vs subconta automática
...
- `20260615180000_billing_asaas.sql` ⏳ aplicar (2026-06-15) — Billing Fase 3: `empresa_cobrancas` (espelho Asaas), `asaas_webhook_eventos` (idempotência), RPCs `billing_creditar_execucoes`/`billing_creditar_tokens`. ⚠️ requer env no Railway (API): `ASAAS_API_KEY`, `ASAAS_ENV`, `ASAAS_WEBHOOK_TOKEN` + configurar webhook no painel Asaas → `POST /billing/webhook/asaas`
