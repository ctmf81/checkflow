...
6. ⏳ **Regra de troca de plano (a implementar)**: troca entre planos só vale no FIM do período vigente (`proximo_plano_id` + `troca_efetiva_em = periodo_fim`; `avancar_periodo_assinatura` já aplica o snapshot). 1ª contratação de pago (vindo de trial/gratuito) = imediata. Hoje o `/billing/assinar` troca IMEDIATO — precisa virar agendado. Implica passo na API na virada do período p/ atualizar/cancelar a assinatura no Asaas (a função SQL não fala com Asaas). Confirmar com usuário: aplica a upgrade também? (recomendado sim, sem pro-rata) — perguntas feitas, sem resposta ainda.
7. ⏳ Fase 4: split parceiro via subconta Asaas (`asaas_wallet_id`, criação automática, split nas cobranças). Decidir: walletId manual (recomendado) vs subconta automática
